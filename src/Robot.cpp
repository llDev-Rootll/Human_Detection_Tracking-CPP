 ...
 */
vector<double> Robot::transformToRobotFrame (vector<Rect> bbox_coords) { //double pixel_to_meter, double area_to_depth) {
    ...
    for (int i=0; i<number_of_boxes; i++) {
        ...
        // Calculate location of point w.r.t robot frame using transformation matrix
        Eigen::Vector4d top_left = transformation_cr * max_location;
        std::cout << "Top Left: " << top_left[0] << top_left[1] << top_left[2] << top_left[3] << std::endl;
        Eigen::Vector4d bottom_right = transformation_cr * min_location;
        std::cout << "Bottom Right: " << bottom_right[0] << bottom_right[1] << bottom_right[2] << bottom_right[3] << std::endl;
    }
    vector<double> dummy;
    return dummy;
}
...
int Robot::detectHumans() {
    ...
    while (true) {
        ...
        // 		 box.x + box.width << "\n Ymax: " << box.y + box.height << std::endl;
